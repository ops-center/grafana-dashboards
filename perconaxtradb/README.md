...
#### Create PerconaXtraDB Metrics Configurations

At first, you have to create a `MetricsConfiguration` object for PerconaXtraDB CR. This `MetricsConfiguration` object is used by Panopticon to generate metrics for the PerconaXtraDB instances.

Run the below command to create the `MetricsConfiguration` object:

```bash
kubectl apply -f https://raw.githubusercontent.com/kubedb/installer/master/charts/kubedb-metrics/templates/perconaxtradb/kubedb-com-perconaxtradb.yaml
```
